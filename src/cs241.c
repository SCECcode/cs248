/**
 * @file cs241.c
 * @brief Main file for cs241 model
 * @version 1.0
 *
 * @section DESCRIPTION
 *
 * Delivers the cs241 model which base from 
 * cs241 that was created to work with mesh created by ucvm2mesh
 *
 */
...
int cs241_init(const char *dir, const char *label) {
    ...
    sprintf(cs241_data_directory, "%s/model/%s/data/%s/", dir, label, cs241_configuration->model_dir);
    ...
    snprintf(cs241_projstr, 64, "+proj=utm +ellps=clrk66 +zone=%d +datum=NAD27 +units=m +no_defs", cs241_configuration->utm_zone);
    if (!(cs241_geo2utm = proj_create_crs_to_crs(PJ_DEFAULT_CTX, "EPSG:4326", cs241_projstr, NULL))) {
        cs241_print_error("Could not set up Proj transformation from EPSG:4326 to UTM.");
    ...
    if(cs241_debug) {
      ...
      fprintf(stderr,"totol height %lf total width %lf\n", cs241_total_height_m, cs241_total_width_m);
      fprintf(stderr,"cos angle %lf sin angle %lf\n", cs241_cos_rotation_angle, cs241_sin_rotation_angle);
    }
...
static int to_geo(double point_u, double point_v, double *lon, double *lat) {
    ...
    PJ_COORD xyzDest = proj_trans(cs241_geo2utm, PJ_INV, xyzSrc);
...
int cs241_query(cs241_point_t *points, cs241_properties_t *data, int numpoints) {
    ...
    int zone = cs241_configuration->utm_zone;
...
if(cs241_debug) { fprintf(stderr,"lon %lf lat %lf\n", points[i].longitude, points[i].latitude); }
if(cs241_debug) { fprintf(stderr,"point_u %lf point_v %lf\n", point_u, point_v); }
...
if(cs241_debug) { fprintf(stderr,"load_x_coord %d load_y_coord %d load_z_coord %d\n", load_x_coord,load_y_coord,load_z_coord); }
        ...
        if(cs241_configuration->interpolation) {
          ...
          if (load_z_coord < 1) {
              ...
              cs241_read_properties(load_x_coord, load_y_coord, load_z_coord, &(data[i]));    // Orgin.
        }
...
void cs241_read_properties(int x, int y, int z, cs241_properties_t *data) {
    ...
    FILE *fp = NULL;
        ...
        if ( strcmp(cs241_configuration->seek_axis, "fast-y") == 0 ||
                 strcmp(cs241_configuration->seek_axis, "fast-Y") == 0 ) { // fast-y,  cs241 
            if(strcmp(cs241_configuration->seek_direction, "bottom-up") == 0) { 
                location = ((long) z * cs241_configuration->nx * cs241_configuration->ny) + (x * cs241_configuration->ny) + y;
if(cs241_debug) {fprintf(stderr,"LOCATION==%d(fast-y, bottom-up)\n", location); }
                } else { // nz starts from 0 up to nz-1
                    location = ((long)((cs241_configuration->nz -1) - z) * cs241_configuration->nx * cs241_configuration->ny) + (x * cs241_configuration->ny) + y;
if(cs241_debug) {fprintf(stderr,"LOCATION==%d(fast-y, not bottom-up)\n", location); }
            }
        } else {  // fast-X, cca data
            if ( strcmp(cs241_configuration->seek_axis, "fast-x") == 0 ||
                     strcmp(cs241_configuration->seek_axis, "fast-X") == 0 ) { // fast-y,  cs241 
                if(strcmp(cs241_configuration->seek_direction, "bottom-up") == 0) { 
                    location = ((long)z * cs241_configuration->nx * cs241_configuration->ny) + (y * cs241_configuration->nx) + x;
if(cs241_debug) {fprintf(stderr,"LOCATION==%d(fast-x, bottom-up)\n", location); }
                    } else { // bottom-up
                        location = ((long)(cs241_configuration->nz - z) * cs241_configuration->nx * cs241_configuration->ny) + (y * cs241_configuration->nx) + x;
if(cs241_debug) {fprintf(stderr,"LOCATION==%d(fast-x, not bottom-up)\n", location); }
                }
            }
        }
    ...
    if (cs241_velocity_model->vp_status == 2) {
        ...
        // Read from file.
        fp = (FILE *)cs241_velocity_model->vp;
        fseek(fp, location * sizeof(float), SEEK_SET);
                float temp;
        fread(&(temp), sizeof(float), 1, fp);
                data->vp=temp;
    ...
    if (cs241_velocity_model->rho_status == 2) {
        ...
        // Read from file.
        fp = (FILE *)cs241_velocity_model->rho;
        fseek(fp, location * sizeof(float), SEEK_SET);
                float temp;
        fread(&(temp), sizeof(float), 1, fp);
                data->rho=temp;
...
int cs241_finalize() {
    ...
    if (cs241_velocity_model) free(cs241_velocity_model);
    if (cs241_configuration) free(cs241_configuration);
...
void cs241_print_error(char *err) {
    fprintf(stderr, "An error has occurred while executing cs241. The error was: %s\n",err);
 ...
 */
static int too_big() {
        long max_size= (long) (cs241_configuration->nx) * cs241_configuration->ny * cs241_configuration->nz;
...
int cs241_try_reading_model(cs241_model_t *model) {
    ...
    sprintf(current_file, "%s/vp.dat", cs241_data_directory);
    ...
    sprintf(current_file, "%s/vs.dat", cs241_data_directory);
    if (access(current_file, R_OK) == 0) {
                if( !too_big( )) { // only if fit
    ...
    sprintf(current_file, "%s/rho.dat", cs241_data_directory);
    if (access(current_file, R_OK) == 0) {
                if(!too_big() ) { // only if fit
    ...
    sprintf(current_file, "%s/qp.dat", cs241_data_directory);
    if (access(current_file, R_OK) == 0) {
                if( !too_big() ) { // only if fit
    ...
    sprintf(current_file, "%s/qs.dat", cs241_data_directory);
    if (access(current_file, R_OK) == 0) {
                if( !too_big() ) { // only if fit
